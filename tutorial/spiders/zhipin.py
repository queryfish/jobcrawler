...
class ZhipinSpider(scrapy.Spider):
    ...
    max_page = 4 #最大页码
    start_urls = [
        "https://www.zhipin.com/mobile/jobs.json?city=101010100&query=chanpin",
    ]
    custom_settings = {
        "ITEM_PIPELINES":{
    ...
    def parse(self, response):
        ...
        if self.current_page < self.max_page:
            self.current_page += 1
            api_url = "https://www.zhipin.com/mobile/jobs.json?city=101010100&query=chanpin"+"&page="+str(self.current_page)
            # time.sleep(int(random.uniform(50, 70)))
